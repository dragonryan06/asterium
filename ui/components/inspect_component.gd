...
func create_tooltip() -> void:
	...
	add_child(tt)
	_tt_lock = true
	find_parent("PlanetGeneration").find_child("UIManager").tooltip_changed.emit()
	_tt_lock = false
...
func _on_tree_entered():
	find_parent("PlanetGeneration").find_child("UIManager").tooltip_changed.connect(_on_tooltip_changed)
